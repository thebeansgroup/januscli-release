...
export function fetchProjectDetails(token) {
  ...
  const log = shell.exec('git remote -v', {silent: this.janus.debug}).stdout;
...
export function fetchPullRequestIDs (token) {
  ...
  const hasTags = shell.exec('git tag -l', {silent: this.janus.debug}).stdout.length;
  ...
  const commits = shell.exec(logCMD, {silent: this.janus.debug}).stdout;
