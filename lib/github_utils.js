...
export function fetchProjectDetails(debug) {
  if (debug == null) debug = false;

  const github_re = /github\.com[\/|:]([-a-z0-9\_]+)\/([-a-z0-9\_]+)/g;
  const log = shell.exec('git remote -v', {silent: !debug}).stdout;
...
export function fetchPullRequestIDs (token, debug) {
  if (debug == null) debug = false;

  const pr_re = /Merge pull request #(\d+)(?!.*release)/g;
  const hasTags = shell.exec('git tag -l', {silent: !debug}).stdout.length;
  ...
  const commits = shell.exec(logCMD, {silent: !debug}).stdout;
